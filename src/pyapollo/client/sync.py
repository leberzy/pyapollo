...
from pyapollo.cache import FileCache, MemoryCache, resolve_cache_root
...
from pyapollo.listeners import ListenerRegistry, Subscription
...
class ApolloClient:
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            autostart: When ``True`` (default), call ``start()`` at end of construction.
    ...
    def start(self) -> ApolloClient:
        """Discover config server, fetch namespaces, and start long-polling."""
        if self._started:
            return self
        self._initialize_config_server()
        try:
            self._repository.sync_all()
        except Exception as exc:
            logger.warning("Initial configuration sync failed: %s", exc)
            self.load_local_cache_file()
        self._mark_ready_if_cached()
        self._repository.start_background()
        self._started = True
        logger.info("Apollo client started for app_id=%s", self._app_id)
        return self

    def stop(self) -> None:
        """Stop long-polling, listener pool, and HTTP transport."""
        if self._started:
            self._repository.stop_background()
            self._listeners.shutdown(wait=False)
        if hasattr(self, "_transport"):
            self._transport.close()
        self._started = False
        logger.info("Apollo client stopped")
    ...
    def _init_transport_stack(self) -> None:
        ...
        self._transport = RequestsTransport(default_timeout=float(self._timeout))
        self._locator = ConfigServiceLocator(
            self._transport,
    ...
    def _switch_config_server_if_discovery(self, exclude: str | None = None) -> None:
        if self._custom_config_server_host:
            return
        self.update_config_server(exclude)

    def _init_repository(self) -> None:
        self._repository = SyncConfigRepository(
            self._api,
            self._locator,
            self._namespaces,
    ...
    def _repository_apply_config(
        ...
        if event and event.changes:
            self._listeners.dispatch(event)
        self._ready = True
    ...
    def add_change_listener(
        ...
        *,
        ...
        Args:
            callback: Receives ``ConfigChangeEvent`` with ADDED/MODIFIED/DELETED details.
            namespaces: Limit to these namespaces; ``None`` means all tracked namespaces.
            keys: Limit to these keys; ``None`` means all changed keys.
        """
    ...
    def _repository_on_fetch_error(self, namespace: str, exc: Exception) -> None:
        data = self._file_cache.load(namespace) or {}
    ...
    def _rebuild_transport_and_repository(self) -> bool:
    ...
    def _init_caches(self, cache_root: str | None = None) -> None:
        ...
        self._file_cache = FileCache(root, self._app_id, self._cluster)

    def start_polling_thread(self) -> None:
        """Deprecated: use ``start()`` or repository background via ``start()``."""
        self._repository.start_background()

    def stop_polling_thread(self) -> None:
        """Deprecated: use ``stop()`` for full shutdown."""
        self._repository.stop_background()
    ...
    def update_local_file_cache(
        ...
        self._file_cache.save(namespace, data, release_key)

    def get_local_file_cache(self, namespace: str = "application") -> dict[str, str]:
        """Get configuration from local file cache."""
        return self._file_cache.load(namespace) or {}

    def update_cache(self, namespace: str, data: dict[str, str]) -> None:
    ...
    def fetch_config_by_namespace(self, namespace: str = "application") -> None:
        ...
        if not self._config_homepage_url:
            ...
            self.update_cache(namespace, self.get_local_file_cache(namespace))
            return
        self._repository.sync_namespace(namespace)
    ...
    def load_local_cache_file(self) -> bool:
        ...
        try:
            for namespace, data in self._file_cache.load_all().items():
                self._memory_cache.set(namespace, data)
            return True
        except Exception as e:
    ...
    def get_service_conf(self) -> list[dict[str, object]]:
        """Get config service list from meta discovery (Java ConfigServiceLocator)."""
        services = self._locator.discover()
        return [
            {
    ...
    def update_config_server(self, exclude: str | None = None) -> str:
        """Update config server via meta discovery with optional exclusion."""
        service = self._locator.choose_one(exclude_homepage=exclude)
    ...
    def update_config(self, **kwargs) -> None:
        ...
        if needs_transport_rebuild:
            resume_background = self._rebuild_transport_and_repository()
